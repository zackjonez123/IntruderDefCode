...
def path(in_path):
    ...
    for i in range(len(dir_list)):
        capt_path = dir_list[i] # path to captured image (from USB cam)
...
def main():
    ...
    # # capt_path = "C:\\Users\\kelly\\Desktop\\IDEs and Sims\\IntruderDef\\pics\\current_img1.jpg" # path to captured image (from USB cam)
    # # grayscale = grayscaleloader(capt_path) # Reads the current image
    # grayscale = path
    # img = transforms.functional.to_tensor(grayscale) # Loads the current image as a tensor
    # img = img[None, :, :, :] # Makes image tensor 4 dimensional for the CNN
